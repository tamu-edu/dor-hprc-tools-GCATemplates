...
################################### COMMANDS ###################################
# This script runs the entire analysis in the $TMPDIR since maker can create tens of thousands of files that reach your file quota
# Files in the $TMPDIR do now count against your quotas. The $TMPDIR is deleted when the job ends.
...
cd -
# copy results gff and fasta files
cp $TMPDIR/${out_prefix}.maker.output/${out_prefix}_master_datastore_index.log ./
cp $TMPDIR/${out_prefix}.all.maker.*{fasta,gff} ./

# if you want a copy of log files, use the following. Note: maker generates one log file per genome contig
#rsync -zarv --prune-empty-dirs --include="*/" --include="*.log" --exclude="*" $TMPDIR/${out_prefix}.maker.output/ ./

# you can review the following log file and look for any tasks in the FAILED state
# out_prefix_master_datastore_index.log 
