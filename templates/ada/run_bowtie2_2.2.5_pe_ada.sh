...
#BSUB -M 2500                   # sets to 1,000MB (~1GB) per process enforceable memory limit. Total job memory = (M * n)
...
########## INPUTS ##########
pe_1='../../../data/sra/m_musculus/mm_genomic_ERR230011_1.fasta.gz'
pe_2='../../../data/sra/m_musculus/mm_genomic_ERR230011_2.fasta.gz'

# use an already prefixed genome found at: /scratch/datasets/ucsc/genome_indexes/
...
########## OUTPUTS #########
output_bam='mm_genomic_ERR230011_pe_aln.bam'

################################### COMMANDS ###################################
# add read group id and other RG values
bowtie2 -p $threads --rg-id "$rg_id" --rg "LB:$rg_library" --rg "SM:$rg_sample" --rg "PL:$rg_platform" -x $genome_index_prefix -1 $pe_1 -2 $pe_2 -f | samtools view -bS - > $output_bam
