...
################################### COMMANDS ###################################
# run maker in the $TMPDIR; job cannot be restarted since $TMPDIR contents are deleted when job ends 
# get pwd and copy all three maker .ctl files to $TMPDIR
mydir=`pwd`
cp maker*.ctl $TMPDIR

# go to the $TMPDIR and run maker
cd $TMPDIR
maker -cpus $threads
...
# copy results files from $TMPDIR to previous working directory
